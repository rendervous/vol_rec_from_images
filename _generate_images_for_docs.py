...
# Generate reference image for volume
fig, axes = plt.subplots(len(reconstructions), 1, figsize=(4, 2*len(reconstructions)), dpi=300)
for i, rec_file in enumerate(reconstructions):
    draw_volume(reference_volume, axes[i])
fig.tight_layout(pad=0.1)
plt.show()

fig, axes = plt.subplots(len(reconstructions), 1, figsize=(4, 2*len(reconstructions)), dpi=300)
for i, rec_file in enumerate(reconstructions):
    rec = torch.load(rec_file, map_location=device)
    draw_volume(rec['sigma_grid'], axes[i])
    axes[i].text(251, 5, names[i], color='white', horizontalalignment='right')
fig.tight_layout(pad=0.1)
plt.show()

fig, axes = plt.subplots(len(reconstructions), 1, figsize=(4, 2*len(reconstructions)), dpi=300)
for i, rec_file in enumerate(reconstructions):
    axes[i].imshow(dataset['radiances'][TEST_IMAGE][128:-128].cpu() ** (1.0/2.2))
    axes[i].invert_yaxis()
    axes[i].axis('off')
fig.tight_layout(pad=0.1)
plt.show()
...
for i, rec_file in enumerate(reconstructions):
    rec = torch.load(rec_file, map_location=device)
    rad = render_volume(rec['sigma_grid'])
    axes[i].imshow(rad[0][128:-128].cpu() ** (1.0/2.2))
    axes[i].invert_yaxis()
    axes[i].axis('off')
fig.tight_layout(pad=0.1)
plt.show()
